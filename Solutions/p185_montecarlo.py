# in this approach I will use a stochastic approach that will try the most probable guess for each digit first and then move on to lower ones
import numpy
import time
import random
...
def check():
    ...
    for i in range(100):
        ...
        liste = []
        while summe < 17:
            number = random.choice(list(input_dict.keys()))
            summe += input_dict[number]
            liste.append(number)
...
def check_guess(n):
    for number in input_dict.keys():
        count = 0
        for i in range(len(str(n))):
            if str(n)[i] == str(number)[i]:
                count += 1

        if count != input_dict[number]:
            return False
...
while True:
    ...
    temp_list = number[:] # I need to use the slice! If I use just temp_list = number it'll only create a reference!!!!
    rand = random.randint(0,len_poss-1)
    ...
    if new_metric < old_metric:
        ...
        choice = numpy.random.choice(numpy.arange(0, 2), p=[1-numpy.exp(-(new_metric - old_metric)/T),numpy.exp(-(new_metric - old_metric)/T)])
        if choice == 1:
            number = list(''.join(temp_list))
            old_metric = new_metric
