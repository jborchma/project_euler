...
while status:
    ...
    closed_list[current] = 1 # Look for the lowest F cost square on the open list and put it in closed_list
    searched[current[0],current[1]] = 2
    del open_list[current]

    for move in moves:
        neighbour = tuple(map(lambda x, y: x + y, current, move))
        if neighbour[0] in range(0,dimension) and neighbour[1] in range(0,dimension) and not neighbour in closed_list: #tests that I'm still in the array
            if searched[neighbour[0],neighbour[1]] < 2:
                if g[neighbour[0],neighbour[1]] > g[current[0], current[1]] + grid[neighbour[0],neighbour[1]]:
                    g[neighbour[0],neighbour[1]] = g[current[0], current[1]] + grid[neighbour[0],neighbour[1]]
                    
                open_list[neighbour] = data[neighbour[0],neighbour[1]] + G_score[neighbour[0],neighbour[1]]
